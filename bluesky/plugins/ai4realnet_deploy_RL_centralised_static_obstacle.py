...
algorithm = 'SAC'
env_name = 'CentralisedStaticObstacleEnv'
def init_plugin():
...
class DeployRL(core.Entity):  
    ...
    def _set_action(self, action):
        ...
        for i in range(len(traf.id)):
            ...
            print(f'Action for aircraft {id} - traf.hdg: {traf.hdg[i]} -> {heading_new} with dh {dh}, traf.tas: {traf.tas[i]} -> {speed_new} with dv {dv}')
