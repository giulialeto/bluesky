...
import numpy as np
import bluesky.plugins.ai4realnet_deploy_RL_tools_v1.StaticObstacleSectorCREnv as RLtools
...
save_dir = 'ai4realnet_deploy_RL_randomised_scen_loading_v1-2/generated_scenarios/v1'
...
class DeployRL(core.Entity):  
    def __init__(self):
        ...
        self.csv_file = (f"output/ai4realnet_deploy_RL_v1_{env_name}_log.csv")
        # print(f'initialised at {self.scn_idx}')
        self.initialise_observation_flag = False
        os.makedirs(f'scenario/{save_dir}', exist_ok=True)
        self.timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

    def reset(self):
        # print(f'reset at {self.scn_idx}')
        ...
        stack.stack('pcall ai4realnet_deploy_RL_randomised_scen_loading_v1-2/sector.scn')
        stack.stack('pcall ai4realnet_deploy_RL_randomised_scen_loading_v1-2/config_screen')
        stack.stack('initialize_scenario')

        stack.stack(f'SAVEIC {save_dir}/{self.timestamp}_{env_name}_{algorithm}_{N_AC}_{RLtools.constants.NUM_OBSTACLES}_{self.scn_idx}')
